...
if($b == "edit") {
	$id = protect($_GET['id']);
	$query = $db->query("SELECT * FROM bit_users WHERE id='$id'");
	if($query->num_rows==0) { header("Location: ./?a=users"); }
	$row = $query->fetch_assoc();
	?>
	<ol class="breadcrumb">
		<li><a href="./">eTransfery Administrator</a></li>
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			if(isset($_POST['btn_save'])) {
				...
				$check_u = $db->query("SELECT * FROM bit_users WHERE username='$username'");
				$check_e = $db->query("SELECT * FROM bit_users WHERE email='$email'");
				...
				elseif($row['email'] !== $email && $check_e->num_rows>0) { echo errro("This email address is already used. Please choose another."); }
					...
					$update = $db->query("UPDATE bit_users SET username='$username',email='$email',password='$npass',status='$status',mobile_number='$mobile_number',email_verified='$email_verified',document_verified='$document_verified',mobile_verified='$mobile_verified' WHERE id='$row[id]'");
			...
			if($c == "delete_document_1") {
				$file = '../'.$row[document_1];
				$update = $db->query("UPDATE bit_users SET document_1='' WHERE id='$row[id]'");
				@unlink($file);
			...
			if($c == "delete_document_2") {
				$file = '../'.$row[document_2];
				$update = $db->query("UPDATE bit_users SET document_2='' WHERE id='$row[id]'");
				@unlink($file);
	...
	<div class="panel panel-danger">
		...
		<div class="panel-body">
			...
			if(isset($_GET['confirm'])) {
				$delete = $db->query("DELETE FROM bit_users WHERE id='$row[id]'");
				$delete = $db->query("DELETE FROM bit_users_earnings WHERE uid='$row[id]'");
				$delete = $db->query("DELETE FROM bit_users_withdrawals WHERE uid='$row[id]'");
