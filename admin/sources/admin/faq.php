...
if($b == "add") {
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			if(isset($_POST['btn_add'])) {
				...
				$check = $db->query("SELECT * FROM bit_faq WHERE question='$question'");
				...
				elseif($check->num_rows>0) { echo error("This question already exists."); }
					...
					$time = time();
					$insert = $db->query("INSERT bit_faq (question,answer,created) VALUES ('$question','$answer','$time')");
	...
	$id = protect($_GET['id']);
	$query = $db->query("SELECT * FROM bit_faq WHERE id='$id'");
		...
		<div class="panel-body">
			...
			if(isset($_POST['btn_save'])) {
				...
				elseif($row['question'] !== $question && $check->num_rows>0) { echo error("This question already exists."); }
					...
					$time = time();
					$update = $db->query("UPDATE bit_faq SET question='$question',answer='$answer',updated='$time' WHERE id='$row[id]'");
					$query = $db->query("SELECT * FROM bit_faq WHERE id='$id'");
					$row = $query->fetch_assoc();
	...
	<div class="panel panel-danger">
		...
		<div class="panel-body">
			...
			if(isset($_GET['confirm'])) {
				$delete = $db->query("DELETE FROM bit_faq WHERE id='$row[id]'");
