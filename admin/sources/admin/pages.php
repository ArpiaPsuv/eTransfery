...
if($b == "add") {
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			if(isset($_POST['btn_add'])) {
				...
				$check = $db->query("SELECT * FROM bit_pages WHERE prefix='$prefix'");
				...
				elseif($check->num_rows>0) { echo error("This prefix is already used. Please choose another. "); }
					...
					$time = time();
					$insert = $db->query("INSERT bit_pages (title,prefix,content,created) VALUES ('$title','$prefix','$content','$time')");
	...
	$id = protect($_GET['id']);
	$query = $db->query("SELECT * FROM bit_pages WHERE id='$id'");
		...
		<div class="panel-body">
			...
			if(isset($_POST['btn_save'])) {
				...
				elseif($row['prefix'] !== $prefix && $check->num_rows>0) { echo error("This prefix is already used. Please choose another. "); }
					...
					$link = '<a href="'.$page.'" target="_blank">'.$page.'</a>';
					$time = time();
					$update = $db->query("UPDATE bit_pages SET title='$title',prefix='$prefix',content='$content',updated='$time' WHERE id='$row[id]'");
					$query = $db->query("SELECT * FROM bit_pages WHERE id='$id'");
					$row = $query->fetch_assoc();
	...
	<div class="panel panel-danger">
		...
		<div class="panel-body">
			...
			if(isset($_GET['confirm'])) {
				$delete = $db->query("DELETE FROM bit_pages WHERE id='$row[id]'");
