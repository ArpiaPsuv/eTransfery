...
<head>
  ...
  <title>eTransfery Authorization</title>
  ...
  <div id="main-wrapper">
    <div class="navbar navbar-inverse" role="navigation">
      <div class="navbar-header">
        <div class="logo"><h1>eTransfery Authorization</h1></div>
      </div>   
    </div>
		...
		<div class="col-sm-12" style="padding:10px;">
			<div style="padding:20px;">
				...
				if(isset($_POST['btn_login'])) {
					...
					$query = $db->query("SELECT * FROM bit_users WHERE username='$username' and password='$password'");
					if($query->num_rows>0) {
						$row = $query->fetch_assoc();
						if($row['status'] == "666") {
							$_SESSION['bit_admin_uid'] = $row['id'];
							header("Location: ./");
						} elseif($row['status'] == "777") {
							$_SESSION['bit_operator_uid'] = $row['id'];
							header("Location: ./");
						} else {
